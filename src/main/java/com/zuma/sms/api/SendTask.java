...
public class SendTask implements Delayed {
	...
	//同步未响应数
	private AtomicInteger syncUnResponse;
	//异步未响应数
	private AtomicInteger asyncUnResponse;
	//已操作数
	...
	public void endHandle() {
		try{
			...
			sendTaskRecord
					...
					.setTotalTime((int)(this.realStartTime - this.realEndTime))
					.setSuccessNum(this.successNum.get())
					.setFailedNum(this.failedNum.get())
					.setSyncUnResponse(this.syncUnResponse.get())
					.setAsyncUnResponse(this.asyncUnResponse.get())
					.setUsedNum(this.usedNum.get())
					.setErrorInfo(CodeUtil.objectToJsonString(result))
	...
	private void shutdown() {
		try {
			...
			executor.shutdown();
			executor = null;
			//修改为 中断 或 成功,如果失败,只会是启动失败
			sendTaskRecordService.updateStatus(sendTaskRecord.getId(),
					this.isInterrupt ?
	 ...
	 * 任务准备
	 */
	public void setup(BlockingQueue<SendTask> closeQueue, FileAccessor fileAccessor) {
		//修改记录状态为运行中
		sendTaskRecordService.updateStatus(sendTaskRecord.getId(), SendTaskRecordStatusEnum.RUN);
		//开始时间
		this.realStartTime = System.currentTimeMillis();
		//修改当前任务状态为 运行中
		this.status = SendTaskStatusEnum.RUN.getCode();
		//获取到号码集合的并发队列
		this.phoneQueue = fileAccessor.readBySendTask(sendTaskRecord.getNumberGruopId());
		//构造线程池
		...
		this.syncUnResponse = new AtomicInteger();
		this.asyncUnResponse = new AtomicInteger();
		this.usedNum = new AtomicInteger();
		//查询通道
		this.channel = channelService.findOne(sendTaskRecord.getChannelId());
		//匹配短信发送器
	...
	@Autowired
	private void init(SendTaskRecordService sendTaskRecordService,ChannelService channelService,FileAccessor fileAccessor) {
